...
const ACCESSORY_MEMBERS = new Map<string, string>([
  ...
  // 칠흑 (인게임 제공: 얼장 루즈 컨트롤 머신 마크 / 눈장 마력이 깃든 안대 / 기계심장 컴플리트 언더컨트롤 /
  //        벨트 몽환의 벨트 / 펜던트 고통의 근원 / 뱃지 창세의 뱃지 / 귀고리 커맨더 포스 이어링 / 반지 거대한 공포 /
  //        포켓 저주받은 마도서 택1 / 엠블렘 미트라의 분노 택1 — 마도서·미트라는 아래 패턴으로)
  ['루즈 컨트롤 머신 마크', '칠흑의 보스'], ['마력이 깃든 안대', '칠흑의 보스'],
  ['몽환의 벨트', '칠흑의 보스'], ['고통의 근원', '칠흑의 보스'], ['창세의 뱃지', '칠흑의 보스'],
  ['커맨더 포스 이어링', '칠흑의 보스'], ['거대한 공포', '칠흑의 보스'],
  ['컴플리트 언더컨트롤', '칠흑의 보스'],
...
// 이름이 변형되는 멤버(마도서 4종·직업별 엠블렘)는 패턴으로
const ACCESSORY_PATTERNS: [RegExp, string][] = [
  [/^저주받은 .*마도서$/, '칠흑의 보스'],
  [/^미트라의 분노/, '칠흑의 보스'],
];
...
export function setBaseOfItem(itemName: string): string | null {
  ...
  for (const [re, base] of ACCESSORY_PATTERNS) if (re.test(itemName)) return base;
...
const SET_SLOTS: Record<string, Set<string>> = {
  ...
  '칠흑의 보스': new Set(['얼굴장식', '눈장식', '기계 심장', '벨트', '펜던트', '뱃지', '귀고리', '반지', '포켓 아이템', '엠블렘']),
