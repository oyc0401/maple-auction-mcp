...
const LINK: Record<string, Pick[]> = {
  ...
  // 카데나 — 상태이상 라인 + 레벨낮은 몬스터 라인 둘 다 보스전 상시 추가뎀(유저 확정 2026-07-12: 오유찬 Lv2 6+6=12).
  // 넥슨 "상태이상 추가 데미지" 필드는 상태이상 라인만 집계 → resting 재구성 상태이상 행에 레벨낮은 라인만큼 +잔차가 정상(오유찬 +6, 티엘 +9).
  '인텐시브 인썰트': [
    { find: '상태 이상에 걸린 몬스터 공격 시 데미지' },
    { find: '캐릭터보다 레벨이 낮은 몬스터 공격 시 데미지', as: '상태 이상에 걸린 몬스터 공격 시 데미지' },
  ],
